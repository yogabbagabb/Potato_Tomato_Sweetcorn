...
def examineData():
    ...
    # The code below checks to see if we have any entries where precip5 is null but vpdave5 is not, or vpdave5 is null but precip5 is not.
    # If False is printed below, then the answer to this question is yes: precip5 is not null iff vpdave5 is not null. Since precipX is null iff
    # all other variants are null, we can say, more generally, that we find: all variants of precip are not null iff all variants of vpdave are not null.
